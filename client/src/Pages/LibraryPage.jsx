...
const SummaryCard = ({ icon: Icon, label, value, loading, accent = false }) => (
  <div className={`flex min-h-[142px] flex-col justify-between rounded-[28px] border p-5 shadow-[0_24px_80px_rgba(0,0,0,0.24)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 ${accent
                ...
                <div className="flex rounded-full border border-[#e8e4db] bg-[#fcf9f2] p-1.5 dark:border-white/[0.08] dark:bg-[#070b12]">
                  {['All', 'Reading', 'Completed'].map(tab => (
                    <button
                      ...
                      className={`min-h-10 flex-1 whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold uppercase tracking-widest transition-all sm:flex-none sm:px-5 ${activeTab === tab
                ...
                {filteredBooks.map((book) => (
                  ...
                  >
                    ...
                    <Link
                      ...
                      className={`mt-4 flex min-h-11 w-full items-center justify-center rounded-2xl px-3 py-3 text-center text-[11px] font-bold uppercase tracking-[0.14em] transition-all ${book.status === 'completed'
